...
test.describe('Product Details Page Tests', () => {
  ...
  test.beforeEach(async ({ page }) => {
    ...
    await page.route(`**/api/book/get-book-by-id/${mockBookId}`, async (route) => {
      await route.fulfill({
        status: 200,
        body: JSON.stringify({
    ...
    await page.route(`**/api/user/get-user-by-id/${mockSellerId}`, async (route) => {
      await route.fulfill({
        status: 200,
        body: JSON.stringify({
          name: 'Nirajan Mahato',
  ...
  test('Should display book details', async ({ page }) => {
    ...
    await expect(page.locator('text=An advanced guide to mastering React.js.')).toBeVisible();
  });
  ...
  test('Should display an error message when product details fail to load', async ({ page }) => {
    await page.route(`**/api/book/get-book-by-id/${mockBookId}`, async (route) => {
      await route.abort();
    });

    await page.goto(`/products/${mockBookId}`);
    await expect(page.locator('text=Error loading product details.')).toBeVisible();
  });
  ...
  test('Should change main image on thumbnail click', async ({ page }) => {
    ...
    await expect(mainImage).toHaveAttribute('src', `/api/uploads/books/image2.jpg`);
  });
});
