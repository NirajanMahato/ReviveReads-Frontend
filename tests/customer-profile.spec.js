...
test.describe('Customer Profile Page Tests', () => {
  ...
  test.beforeEach(async ({ page }) => {
    ...
    await page.route(`**/api/user/get-user-by-id/${mockUserId}`, async (route) => {
      await route.fulfill({
        status: 200,
        body: JSON.stringify({
          name: 'Nirajan Mahato',
  ...
  test('Should display error message on fetch failure', async ({ page }) => {
    ...
    await page.goto(`/customerprofile/${mockUserId}`);
    await expect(page.locator('text=Failed to fetch user details.')).toBeVisible();
  });

  // ✅ Test: Block user button should be visible
  test('Should display Block User button', async ({ page }) => {
    await expect(page.locator('button', { hasText: 'Block user' })).toBeVisible();
  });
});
